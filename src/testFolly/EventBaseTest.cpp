...
// ./testFolly --gtest_filter=autoPtrTest.*
...
void scheduleEvents(EventBase* eventBase, int fd, ScheduledEvent* events) {
    for (ScheduledEvent* ev = events; ev->milliseconds > 0; ++ev) {
        eventBase->tryRunAfterDelay(std::bind(&ScheduledEvent::perform, ev, fd), ev->milliseconds);
    }
}
...
public:
    ...
    struct EventRecord {
        EventRecord(uint16_t events, size_t bytesRead, size_t bytesWritten) :
                events(events), timestamp(), bytesRead(bytesRead), bytesWritten(bytesWritten) {
        }
...
/**
 * Test a READ event
 * ./testFolly --gtest_filter=EventBaseTest.ReadEvent
 */
TEST(EventBaseTest, ReadEvent) {
    ...
    ASSERT_EQ(handler.log.size(), 0);

    // Register timeouts to perform two write events
    ScheduledEvent events[] = { { 10, EventHandler::WRITE, 2345, 0 }, { 160, EventHandler::WRITE, 99, 0 },
            { 0, 0, 0, 0 }, };
...
//    LOG(INFO) << "1111111111111111111111111111111111111";
    ...
    T_CHECK_TIMEOUT(start, handler.log[0].timestamp, milliseconds(events[0].milliseconds), milliseconds(90));

    ASSERT_EQ(handler.log[0].bytesRead, events[0].length);
    ASSERT_EQ(handler.log[0].bytesWritten, 0);

    T_CHECK_TIMEOUT(start, end, milliseconds(events[1].milliseconds), milliseconds(30));
...
/**
 * Test (READ | PERSIST)
 * ./testFolly --gtest_filter=EventBaseTest.ReadPersist
 */
TEST(EventBaseTest, ReadPersist) {
    ...
    ScheduledEvent events[] = { { 10, EventHandler::WRITE, 1024, 0 }, { 20, EventHandler::WRITE, 2211, 0 }, { 30,
            EventHandler::WRITE, 4096, 0 }, { 100, EventHandler::WRITE, 100, 0 }, { 0, 0, 0, 0 }, };
    scheduleEvents(&eb, sp[1], events);
    ...
    for (int n = 0; n < 3; ++n) {
        ...
        T_CHECK_TIMEOUT(start, handler.log[n].timestamp, milliseconds(events[n].milliseconds));
...
TEST(EventBaseTest, ReadImmediate) {
    ...
    ScheduledEvent events[] = { { 10, EventHandler::WRITE, 2345, 0 }, { 0, 0, 0, 0 }, };
    scheduleEvents(&eb, sp[1], events);
    ...
    T_CHECK_TIMEOUT(start, handler.log[1].timestamp, milliseconds(events[0].milliseconds));
...
/**
 * Test a WRITE event
 * ./testFolly --gtest_filter=EventBaseTest.WriteEvent
 */
TEST(EventBaseTest, WriteEvent) {
    EventBase eb;
    SocketPair sp;

    // Fill up the write buffer before starting 写到buffer满 返回EAGAIN 为止
    ...
    ScheduledEvent events[] = { { 10, EventHandler::READ, 0, 0 }, { 60, EventHandler::READ, 0, 0 }, { 0, 0, 0, 0 }, };
    scheduleEvents(&eb, sp[1], events);
    ...
    T_CHECK_TIMEOUT(start, handler.log[0].timestamp, milliseconds(events[0].milliseconds));
...
/**
 * Test (WRITE | PERSIST)
 * ./testFolly --gtest_filter=EventBaseTest.WritePersist
 */
TEST(EventBaseTest, WritePersist) {
    ...
    ScheduledEvent events[] = { { 10, EventHandler::READ, 0, 0 }, { 40, EventHandler::READ, 0, 0 }, { 70,
            EventHandler::READ, 0, 0 }, { 100, EventHandler::READ, 0, 0 }, { 0, 0, 0, 0 }, };
    scheduleEvents(&eb, sp[1], events);
...
TEST(EventBaseTest, WriteImmediate) {
    ...
    ScheduledEvent events[] = { { 10, EventHandler::READ, 0, 0 }, { 0, 0, 0, 0 }, };
    scheduleEvents(&eb, sp[1], events);

    // Schedule a timeout to unregister the handler
    int64_t unregisterTimeout = 40;
    eb.tryRunAfterDelay(std::bind(&TestHandler::unregisterHandler, &handler), unregisterTimeout);
 ...
 * ./testFolly --gtest_filter=EventBaseTest.ReadWrite
 */
TEST(EventBaseTest, ReadWrite) {
    ...
    ScheduledEvent events[] =
            { { 10, EventHandler::WRITE, 2345, 0 }, { 40, EventHandler::READ, 0, 0 }, { 0, 0, 0, 0 }, };
    scheduleEvents(&eb, sp[1], events);
 ...
 * ./testFolly --gtest_filter=EventBaseTest.WriteRead
 */
TEST(EventBaseTest, WriteRead) {
    ...
    ScheduledEvent events[] = { { 10, EventHandler::READ, 0, 0 }, { 40, EventHandler::WRITE, sock1WriteLength, 0 }, { 0,
            0, 0, 0 }, };
    scheduleEvents(&eb, sp[1], events);
...
TEST(EventBaseTest, ReadWriteSimultaneous) {
    ...
    ScheduledEvent events[] = { { 10, EventHandler::READ | EventHandler::WRITE, 0, 0 }, { 0, 0, 0, 0 }, };
    scheduleEvents(&eb, sp[1], events);
...
TEST(EventBaseTest, ReadWritePersist) {
    ...
    // Register for read and write events
    TestHandler handler(&eb, sp[0]);
    handler.registerHandler(EventHandler::READ | EventHandler::WRITE | EventHandler::PERSIST);

    // Register timeouts to perform several reads and writes
    ScheduledEvent events[] = { { 10, EventHandler::WRITE, 2345, 0 }, { 20, EventHandler::READ, 0, 0 }, { 35,
            EventHandler::WRITE, 200, 0 }, { 45, EventHandler::WRITE, 15, 0 }, { 55, EventHandler::READ, 0, 0 }, { 120,
            EventHandler::WRITE, 2345, 0 }, { 0, 0, 0, 0 }, };
    scheduleEvents(&eb, sp[1], events);
...
TEST(EventBaseTest, ReadPartial) {
    ...
    ScheduledEvent events[] = { { 10, EventHandler::WRITE, (3 * readLength) + (readLength / 2), 0 }, { 0, 0, 0, 0 }, };
    ...
    for (int n = 0; n < 3; ++n) {
        ASSERT_EQ(handler.log[n].events, EventHandler::READ);
        T_CHECK_TIMEOUT(start, handler.log[n].timestamp, milliseconds(events[0].milliseconds));
    ...
    T_CHECK_TIMEOUT(start, handler.log[3].timestamp, milliseconds(events[0].milliseconds));
...
TEST(EventBaseTest, DestroyHandler) {
    ...
    eb.tryRunAfterDelay(std::bind(checkReadUntilEmpty, sp[1], initialBytesWritten), 10);
...
struct RunInThreadData {
    RunInThreadData(int numThreads, int opsPerThread) :
            opsPerThread(opsPerThread), opsToGo(numThreads * opsPerThread) {
    }
...
struct RunInThreadArg {
    RunInThreadArg(RunInThreadData* data, int threadId, int value) :
            data(data), thread(threadId), value(value) {
    }
...
//./testFolly --gtest_filter=EventBaseTest.mytest1
TEST(EventBaseTest, mytest1) {
    std::cout << "main thread id: " << pthread_self() << std::endl;
    constexpr uint32_t numThreads = 5;
    constexpr uint32_t opsPerThread = 10;
    RunInThreadData data(numThreads, opsPerThread);

    deque<std::thread> threads;
    SCOPE_EXIT {   //作用域结束时执行代码段！
...
/**
 *
 * ./testFolly --gtest_filter=EventBaseTest.myTest2
 */
TEST(EventBaseTest, myTest2) {
    std::cout << "main thread id: " << pthread_self() << std::endl;
    EventBase base;
    ...
    base.runInEventBaseThread([&]() {
        std::cout<<"This will be printed in thread"  << pthread_self() << std::endl;
    });

    base.tryRunAfterDelay(std::bind(&EventBase::terminateLoopSoon, &base), 3000);
    thread1.join();
}

//./testFolly --gtest_filter=EventBaseTest.RunInThread
TEST(EventBaseTest, RunInThread) {
    ...
    // stop.
    data.evb.tryRunAfterDelay(std::bind(&EventBase::terminateLoopSoon, &data.evb), 3000);
    ...
    auto timeTaken = std::chrono::duration_cast<milliseconds>(end.getTime() - start.getTime());
    ...
    for (deque<pair<int, int> >::const_iterator it = data.values.begin(); it != data.values.end(); ++it) {
...
public:
    CountedLoopCallback(EventBase* eventBase, unsigned int count, std::function<void()> action =
            std::function<void()>()) :
...
//./testFolly --gtest_filter=EventBaseTest.RepeatedRunInLoop
...
//c1 c2 will run alternately
TEST(EventBaseTest, RunInLoopStopLoop) {
    EventBase eventBase;

    CountedLoopCallback c1(&eventBase, 20);
    CountedLoopCallback c2(&eventBase, 10, std::bind(&EventBase::terminateLoopSoon, &eventBase));
...
//./testFolly --gtest_filter=EventBaseTest.messageAvailableException
...
TEST(EventBaseTest, TryRunningAfterTerminate) {
    EventBase eventBase;
    CountedLoopCallback c1(&eventBase, 1, std::bind(&EventBase::terminateLoopSoon, &eventBase));
    eventBase.runInLoop(&c1);
    eventBase.loopForever();
    bool ran = false;
    eventBase.runInEventBaseThread([&]() {
...
TEST(EventBaseTest, CancelRunInLoop) {
    ...
    std::function<void()> cancelC1Action = std::bind(&EventBase::LoopCallback::cancelLoopCallback, &c1);
    std::function<void()> cancelC2Action = std::bind(&EventBase::LoopCallback::cancelLoopCallback, &c2);
...
class TerminateTestCallback: public EventBase::LoopCallback, public EventHandler {
public:
    TerminateTestCallback(EventBase* eventBase, int fd) :
            EventHandler(eventBase, fd), eventBase_(eventBase), loopInvocations_(0), maxLoopInvocations_(0), eventInvocations_(
                    0), maxEventInvocations_(0) {
    }
...
public:
    ...
    explicit IdleTimeTimeoutSeries(EventBase *base, std::deque<std::uint64_t>& timeout) :
...
TEST(EventBaseTest, IdleTime) {
    ...
    int64_t testStart = duration_cast<microseconds>(std::chrono::steady_clock::now().time_since_epoch()).count();
...
TEST(EventBaseTest, LoopKeepAlive) {
    ...
    bool done = false;
    std::thread
    t([&, loopKeepAlive = evb.getKeepAliveToken() ]() mutable {
...
TEST(EventBaseTest, LoopKeepAliveWithLoopForever) {
...

    {
        ...
        ev->runInEventBaseThreadAndWait([&ev, &keepAlive] {keepAlive = ev->getKeepAliveToken();});
...
TEST(EventBaseTest, RequestContextTest) {
    ...
    {
        //in RequestContextScopeGuard Constructor, save old RequestContext and create a new one
        //in RequestContextScopeGuard destructor , set back original RequestContest and withdraw the new one

...
void foo(int x) {
    // do something with x
    cout << "in foo thread : " << pthread_self()<<endl;
   sleep(5);
   cout << "foo(" << x << ")" << endl;
}


//./testFolly --gtest_filter=Future.myFutureTest
TEST(Future, myFutureTest) {

    cout << "in main thread : " << pthread_self()<<endl;
    cout << "making Promise" << endl;
    Promise<int> p;
    Future<int> f = p.getFuture();
    f.then(foo);  //When this Future has completed, execute function foo

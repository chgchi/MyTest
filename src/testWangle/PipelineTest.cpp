...
TEST(PipelineTest, RealHandlersCompile) {
    ...
    {
        auto pipeline =
                StaticPipeline<IOBufQueue&, std::unique_ptr<IOBuf>, AsyncSocketHandler, OutputBufferingHandler>::create(
                        AsyncSocketHandler(socket), OutputBufferingHandler());
    ...
    // dynamic
    {
        ...
        (*pipeline).addBack(AsyncSocketHandler(socket)).addBack(OutputBufferingHandler()).finalize();
        EXPECT_TRUE(pipeline->getHandler<AsyncSocketHandler>());
        EXPECT_TRUE(pipeline->getHandler<OutputBufferingHandler>());
    }
}
...
TEST(PipelineTest, FireActions) {
    ...
    auto pipeline = StaticPipeline<int, int, IntHandler, IntHandler2>::create(&handler1, &handler2);
...
TEST(Pipeline, MissingInboundOrOutbound) {
    ...
    EXPECT_THROW(pipeline->readException(exception_wrapper(std::runtime_error("blah"))), std::invalid_argument);
...
TEST(Pipeline, DynamicConstruction) {
    {
        ...
        EXPECT_NO_THROW(
                (*pipeline).addFront(IntToStringHandler { }).addFront(StringToIntHandler { }).addBack(
                        StringToIntHandler { }).addBack(IntToStringHandler { }).finalize());
    }
}
